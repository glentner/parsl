...
class RSyncStaging(Staging, RepresentationMixin):
    ...
    initialization must include an appropriate SSH key configuration.
    """
...
def in_task_stage_in_wrapper(func, file, working_dir, hostname):
    def wrapper(*args, **kwargs):
        ...
        r = os.system("rsync {hostname}:{permanent_filepath} {worker_filepath}".format(hostname=hostname, permanent_filepath=file.path, worker_filepath=file.local_path))
        if r != 0:
...
def in_task_stage_out_wrapper(func, file, working_dir, hostname):
    def wrapper(*args, **kwargs):
        ...
        r = os.system("rsync {worker_filepath} {hostname}:{permanent_filepath}".format(hostname=hostname, permanent_filepath=file.path, worker_filepath=file.local_path))
        if r != 0:
            raise RuntimeError("rsync returned {}, a {}".format(r, type(r)))
